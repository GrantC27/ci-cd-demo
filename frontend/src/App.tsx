...
import 'firebase/compat/auth'; // Import for side effects and types like firebase.User
import { auth as firebaseAuth } from './firebase'; // firebaseAuth will be firebase.auth.Auth instance

// pdfjs-dist and mammoth are imported dynamically within functions
...
interface EmailVerificationStatus {
  pending: boolean;
  message: string | null;
  showResendButtonForUser: firebase.User | null; // Changed to firebase.User
}

const App: React.FC = () => {
  const [firebaseUser, setFirebaseUser] = useState<firebase.User | null>(null); // Changed to firebase.User
  ...
  useEffect(() => {
    const setupPdfJs = async () => {
      try {
        ...
        // The workerSrc path needs to be a URL that Vite can resolve and serve.
        pdfJsModule.GlobalWorkerOptions.workerSrc = new URL('pdfjs-dist/build/pdf.worker.min.mjs', import.meta.url).toString();
      } catch (error) {
        console.error("Failed to load PDF.js module or set up worker:", error);
        setGeneralError("PDF functionality might be impaired. Failed to load PDF library components.");
      }
    };
    setupPdfJs();
  }, []);
  ...
  const fetchAndStoreUserProfile = async (user: firebase.User) => { // Changed to firebase.User
    try {
  ...
  useEffect(() => {
    const unsubscribe = firebaseAuth.onAuthStateChanged(async (fbAuthUser) => { // fbAuthUser will be firebase.User | null
  ...
  const handleAuthAttempt = async (email: string, mode: 'login' | 'signup', password?: string): Promise<{ success: boolean; message?: string; requiresVerification?: boolean; user?: firebase.User | null}> => { // Changed to firebase.User
    setGeneralError(null);
    setAuthActionMessage(null);
    try {
      if (mode === 'signup') {
  ...
  const handleCvFileChange = useCallback(async (file: File | null) => {
    ...
    if (file) {
      ...
      try {
        // Libraries (pdfjs-dist, mammoth) are dynamically imported within parseCvFile
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-neutral-light to-gray-200 text-neutral-darker">
      ...
      {showBuyCreditsModal && currentUser && firebaseUser && ( 
        <BuyCreditsModal
