...
interface AuthModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onAuthAttempt: (email: string, mode: AuthMode, password?: string) => Promise<{ success: boolean; message?: string; requiresVerification?: boolean; user?: firebase.User | null }>;
...
export const AuthModal: React.FC<AuthModalProps> = ({ mode, isOpen, onClose, onAuthAttempt, switchMode }) => {
  ...
  const [postAuthMessage, setPostAuthMessage] = useState<string | null>(null); // For messages like "verification email sent"
    ...
    try {
      ...
      if (result.success) {
        if (result.requiresVerification && result.message) {
          // Signup success, verification required
          setPostAuthMessage(result.message);
          // Don't close modal, user needs to see this message. They can close it manually.
        } else {
          // Login success and verified
          onClose(); // Close modal on successful login
        }
      } else { // result.success is false
        setError(result.message || `An error occurred during ${mode}.`);
        if (result.requiresVerification && result.message) {
          // This case is for login attempt with unverified email.
          // Error message is already set. Modal stays open.
          // User will close, then App.tsx banner will show resend option.
        }
      }
    } catch (apiError: any) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && !postAuthMessage && ( // Don't show error if postAuthMessage (like success) is shown
        ...
        {!postAuthMessage && ( // Hide form if showing a success message like "verification sent"
        ...
        {!postAuthMessage && ( // Hide switch mode if showing post auth message
