...
interface BuyCreditsModalProps {
  ...
  firebaseUser: firebase.User | null; // For UID and ID token
  // onPurchase is no longer directly invoked by this modal for Stripe.
  // Credits are updated via backend webhook from Stripe. UI reflects on next profile fetch.
}
...
export const BuyCreditsModal: React.FC<BuyCreditsModalProps> = ({ 
    ...
    try {
      ...
      const response = await fetch(LAMBDA_CREATE_CHECKOUT_SESSION_ENDPOINT, {
        ...
        body: JSON.stringify({ 
            ...
            priceId: selectedPkg.priceId // Send priceId to backend
        }),
      });
  ...
  return (
    <div 
        ...
        onClick={() => { if (!isLoading) onClose();}} // Prevent close while loading
        role="dialog"
        aria-modal="true"
